...
String _fmt(dynamic v, int decimals) => _n(v).toStringAsFixed(decimals);
                        ...
                        Row(children: [
                          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(s.totalSpent.toUpperCase(), style: context.af(
                                fontSize: 9, letterSpacing: 1.1, color: Colors.white54)),
          ...
          if (_loading)
            ...
            SliverToBoxAdapter(child: Padding(
              ...
              child: Row(children: [
                ...
                Expanded(child: _StatCard(icon: Icons.pin_drop_outlined,
                  color: const Color(0xFF7C3AED), label: s.stops,
                  value: '${_logs.length}')),
              ]),
            )),
            ...
            if (_logs.isNotEmpty)
              SliverToBoxAdapter(child: Padding(
                ...
                child: Text(s.recentPurchases.toUpperCase(), style: context.af(
...
class _FuelCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final vendor     = log['vendor_name'] as String? ?? s.fuelStop;
        ...
        child: Column(children: [
          Row(children: [
            Container(width: 46, height: 46,
              decoration: BoxDecoration(color: _green.withValues(alpha: 0.10),
                borderRadius: BorderRadius.circular(13)),
              child: const Icon(Icons.local_gas_station_rounded, color: _green, size: 22)),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(vendor, style: context.af(fontSize: 14, fontWeight: FontWeight.w700, color: _navy)),
              Text(city.isNotEmpty ? '$city • $jur' : jur,
                  style: context.af(fontSize: 12, color: _grey)),
              const SizedBox(height: 6),
              Row(children: [
                _Chip('$gallons gal', _blue.withValues(alpha: 0.08), _blue, context),
                const SizedBox(width: 6),
                _Chip('\$$ppg/gal', Colors.orange.withValues(alpha: 0.08),
                    Colors.orange.shade700, context),
              ]),
            ])),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('\$$cost', style: context.af(fontSize: 17, fontWeight: FontWeight.w800, color: _green)),
          ...
          GestureDetector(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 6),
              Text(hasReceipt ? s.receiptAttached : s.uploadReceipt,
...
Widget _Chip(String l, Color bg, Color text, BuildContext context) => Container(
  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
  decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(6)),
  child: Text(l, style: context.af(fontSize: 10, fontWeight: FontWeight.w600, color: text)));
  ...
  Widget build(BuildContext context) {
    ...
    return Center(child: Column(
      ...
      Container(width: 72, height: 72,
        ...
        child: Icon(Icons.local_gas_station_outlined, size: 34, color: Colors.grey.shade400)),
      const SizedBox(height: 14),
      Text(s.noFuelLogsYet, style: context.af(
          fontSize: 15, fontWeight: FontWeight.w600, color: _grey)),
      const SizedBox(height: 4),
      Text(s.tapToAddFirst, style: context.af(fontSize: 12, color: Colors.grey.shade400)),
    ...
    return Container(
      ...
      child: _uploading
          ...
          : Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(s.uploadReceipt, style: context.af(
                  fontSize: 17, fontWeight: FontWeight.w800, color: _navy)),
              const SizedBox(height: 20),
              _UploadOption(icon: Icons.camera_alt_outlined,
                label: s.takePhoto, onTap: () => _pick(ImageSource.camera)),
              const SizedBox(height: 10),
              _UploadOption(icon: Icons.photo_library_outlined,
                label: s.chooseFromGallery, onTap: () => _pick(ImageSource.gallery)),
            ]),
    );
  }
}
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(width: 10),
        Text(label, style: context.af(fontSize: 14, fontWeight: FontWeight.w600, color: _navy)),
      ]),
    ),
  );
}
