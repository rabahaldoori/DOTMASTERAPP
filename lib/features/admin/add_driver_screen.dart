...
class _AddDriverScreenState extends State<AddDriverScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_pass.text != _passConfirm.text) {
      _showErr('Passwords do not match'); return;
    }
  ...
  InputDecoration _deco(String label, {String? hint, Widget? suffix, Widget? prefix}) =>
    InputDecoration(
      ...
      prefixIcon: prefix,
      suffixIcon: suffix,
      labelStyle: GoogleFonts.inter(fontSize: 13, color: _slate),
      hintStyle: GoogleFonts.inter(fontSize: 13, color: const Color(0xFFBDC7D4)),
      filled: true,
    ...
    GestureDetector(
      ...
      child: AbsorbPointer(
        child: TextFormField(
          ...
          validator: req ? (_) => val == null ? 'Select a date' : null : null,
        ),
      ),
    );
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Hire New Driver',
                        style: GoogleFonts.inter(
                            fontSize: 20, fontWeight: FontWeight.w800,
                            color: Colors.white)),
                    Text('Enter driver details and credentials.',
                        style: GoogleFonts.inter(
                            fontSize: 12,
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _card([
                  _row(
                    _text(_first, 'First Name', hint: 'John', req: true),
                    _text(_last,  'Last Name',  hint: 'Doe',  req: true),
                  ),
                  const SizedBox(height: 12),
                  _row(
                    _text(_phone, 'Phone', hint: '+1 555-000-1234',
                        kb: TextInputType.phone,
                        prefix: const Icon(Icons.phone_outlined, size: 17, color: _slate)),
                    _text(_email, 'Email', hint: 'driver@company.com',
                ...
                // ── CDL ─────────────────────────────────────────────────
                _section('CDL Information', Icons.badge_outlined),
                _card([
                  _row(
                    _text(_cdlNum, 'CDL Number', hint: 'TX-49201-992', req: true),
                    _text(_cdlState, 'CDL State', hint: 'e.g. TX', req: true),
                  ),
                  const SizedBox(height: 12),
                  _row(
                    _datePicker('CDL Expiration *', _cdlExpiry, true, req: true),
                    _datePicker('Hire Date *', _hireDate, false, req: true),
                  ),
                ]),

                // ── Assignment ──────────────────────────────────────────
                _section('Truck Assignment', Icons.local_shipping_outlined),
                _card([
                  DropdownButtonFormField<int?>(
                    value: _assignedTruckId,
                    isExpanded: true,
                    menuMaxHeight: 220,
                    decoration: _deco('Assigned Truck',
                ...
                // ── Password ─────────────────────────────────────────────
                _section('Account Credentials', Icons.lock_outline_rounded),
                _card([
                  _text(_pass, 'App Password', hint: 'Min. 8 characters',
                  ...
                  const SizedBox(height: 12),
                  _text(_passConfirm, 'Confirm Password', hint: 'Re-enter password',
